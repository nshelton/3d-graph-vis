...
ForceLayout = function ( scene ) {
	...
	this.pushEdge = function(a, b, param) {
		var c_index = param.type == "like" ? 0 : param.type == "comment" ? 1 : 2
        ...
        // console.log(this.graph.nodes[a])
        //geometry.vertices[0].copy(this.graph.nodes[a].position);
        
        // geometry.vertices[1].copy(this.graph.nodes[a].position);
        // geometry.vertices[2].copy(this.graph.nodes[a].position);
        // geometry.vertices[3].copy(this.graph.nodes[a].position);
        // geometry.vertices[4].copy(this.graph.nodes[a].position);

        //geometry.vertices[5].copy(this.graph.nodes[b].position);
	...
	this.updateEdgePositions = function(g) {
		...
		for (var i = 0; i < g.edges.length; i ++) {
			...
			var noise = new THREE.Vector3(param.bump_mag);
			// dir.subVectors(a.position, b.position);

			l.geometry.vertices[0] = a.position;

			l.geometry.vertices[1].lerpVectors(a.position, b.position, param.bump_pos + param.bump_length) ;
			l.geometry.vertices[2].lerpVectors(a.position, b.position, param.bump_pos + param.bump_length).add(noise) ;
			l.geometry.vertices[3].lerpVectors(a.position, b.position, param.bump_pos - param.bump_length).add(noise) ;
